...
- Define an step model with dependency ordering and unified execution contract.
- Support five OpenSpec-style steps: `proposal`, `specs`, `design`, `tasks`, and `apply`.
- Define run-state persistence and per-step logs for resumable execution.
...
### Decision 2: Two execution backends behind one step contract
...
- Makes execution auditable and debuggable per step.
...
- [Risk] Step type definitions drift from actual implementation. → Mitigation: lock `v1.0.0` step I/O contracts and validate at runtime.
- [Risk] Skill outputs are inconsistent and hard to parse. → Mitigation: require structured output envelope for every skill step.
...
- [Risk] `apply` step scope becomes ambiguous (implementation vs verification). → Mitigation: document v1.0.0 boundary and add explicit follow-up step types in future versions.

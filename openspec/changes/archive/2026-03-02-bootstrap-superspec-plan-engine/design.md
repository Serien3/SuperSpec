...
**Goals:**
- Define a stable `plan.json` schema (`superspec.plan/v0.1`) scoped to a single change.
...
### Decision 3: Sequential execution with explicit dependencies for v0.1
...
Alternatives considered:
- Full DAG executor in v0.1. Rejected as premature complexity.
...
- [Risk] Action type definitions drift from actual implementation. → Mitigation: lock `v0.1` action I/O contracts and validate at runtime.
...
- [Risk] `apply` action scope becomes ambiguous (implementation vs verification). → Mitigation: document v0.1 boundary and add explicit follow-up action types in future versions.

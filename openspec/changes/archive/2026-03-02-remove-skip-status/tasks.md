...
- [x] 1.1 Update protocol requirement text and examples to remove `SKIPPED` as an allowed step terminal state.
...
- [x] 2.4 Ensure terminalization logic reaches a bounded terminal state without leaving dependency-blocked steps pending indefinitely.
...
- [x] 3.3 Add validation tests that reject `onFail: skip_dependents` in plan defaults and step overrides.

...
The system MUST use fail-fast terminal semantics for step failure handling.

#### Scenario: Reported failure always halts workflow
- **WHEN** any step failure is reported through the protocol
- **THEN** the workflow enters terminal `failed`
- **AND** no continuation policy is applied for further autonomous step execution
...
**Migration**: Remove `defaults.retry` and step-level `retry` from plans/workflows; rely on fail-fast handling and human-guided reruns.

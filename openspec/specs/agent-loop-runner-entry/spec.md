...
Define standardized agent guidance for running SuperSpec workflow-backed changes end-to-end using the pull protocol loop.
## Requirements
### Requirement: Agent loop execution guidance
The system MUST provide standardized guidance (via skill and/or AGENT.md) that instructs an external agent to execute the protocol loop for a change until the workflow reaches a terminal state.

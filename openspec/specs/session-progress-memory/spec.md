...
The system MUST require a change name, a commit subject summary, a commit body narrative, and a next-step note when running `superspec git commit`.
...
#### Scenario: Missing structured commit fields are rejected
- **WHEN** a user omits any required field among `--summary`, `--details`, or `--next`
...
The system MUST stage repository changes before committing, while excluding change runtime state files, and MUST merge committed file paths into the target change execution runtime.
...
- **AND** the command excludes the target change `execution/state.json` and `execution/events.log` from auto-staging
...
- **AND** the entry wraps details text between `<!-- superspec:details:start -->` and `<!-- superspec:details:end -->`
